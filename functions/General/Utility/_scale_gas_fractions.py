import numpy as np
# TODO: Add check that ensures that dict or array supplied is 6 in length - i.e. should contain all gas species (even
#  if some are zero). Add better documentation - currently have the two functions in one. Turn gas_fractions_format
#  into kwargs with the optional "decimals" or "percentages"
...
def scale_gas_fractions(gas_fractions, gas_fractions_format = "decimals"):
    ...
    Parameters
    ----------
    gas_fractions: dict or array[float]
    ...
    if isinstance(gas_fractions, dict) == True:
        ...
        for iterations in np.arange(len(gas_fractions[list(gas_fractions)[0]])):
            ...
            if gas_fractions_format == "percentages":
                ...
                print("Check whether gas fractions have been provided at right format")
